...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[BrandAnalyze] POST called: ${video_ids?.length || 0} videos, campaign=${campaign_id}, force=${force}`)
    ...
    console.log(`[BrandAnalyze] Campaign brands: ${campaignBrands.length}`)
    ...
    for (const video of videos) {
      try {
        ...
        // Analyze with AI
        console.log(`[BrandAnalyze] ${video.youtube_id} transcript found (${transcript.text.length} chars, ${transcript.language})`)
        ...
        console.log(`[BrandAnalyze] ${video.youtube_id} detected ${detections.length} candidates, ${detectedBrands.length} high-confidence: ${detectedBrands.join(', ') || 'none'}`)
        if (detectedBrands.length > 0) {
          const videoRow = await queryOne('SELECT tags FROM videos WHERE id = $1', [video.id])
          const currentTags = Array.isArray(videoRow?.tags) ? videoRow.tags : []
          const mergedTags = [...new Set([...currentTags, ...detectedBrands])]
        ...
        console.error(`Failed to analyze video ${video.youtube_id}:`, videoErr?.message || videoErr)
        results.push({
          youtube_id: video.youtube_id, status: 'error',
