...
export default function VideosTab() {
  ...
  const videosQuery = useQuery({
    queryKey: ['videos-tab', activeCampaignId, page, sort, debouncedSearch, tab, brandFilter, channelFilter, ownershipFilter, showAll],
    queryFn: async () => {
      if (showAll) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const endpoint = showAll ? '/api/videos/campaign' : '/api/videos/leaderboard'
      const res = await fetch(`${endpoint}?${params}`)
      const d = await res.json()
      const allVideos = d.data || []

      const headers = 'Title,Channel,Views,Rank,Keywords,Brand Tags,Duration,Published,Ownership'
      const rows = allVideos.map((v: any) => [
        `"${(v.title || '').replace(/"/g, '""')}"`, `"${v.channel_name || ''}"`,
        String(v.view_count || 0), String(v.best_rank || ''),
        String(v.keyword_count || 0),
        `"${(v.brands || v.tags || []).join(', ')}"`,
        `"${v.duration || ''}"`, v.published_at || '', v.is_ours ? 'Yes' : 'No',
      ])
